...
export default function BuyPaymentDettails({
  paymentDetails,
  data,
  closePaymentPopup
}) {
  ...
  const handlePaymentApprove = () => {
    ...
    if (data?.productID) {
      fetch(
        `${process.env.REACT_APP_API_URL}/product/${data?.productID}/koyel-item/payment/approve/winner/${data?.bidderId}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json"
          },
          body: JSON.stringify({ itemId, paymentDetails })
        }
      )
        .then(res => res.json())
        .then(data => {
